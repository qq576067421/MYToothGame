...
public class PlayerTextuerShow : MonoBehaviour
{
    ...
    public void ShowCameraImage()
    {
        ...
        if (PlayerMatchView.Instance.PlayerMatchViewMode == PlayerMatchViewMode.PartitionView)
        {
            ...
            int maxSlots = Mathf.Min(4, _cameraTextureViewRectPartitionView.Length);
            for (int i = 0; i < maxSlots; i++)
            {
                _cameraTextureViewRectPartitionView[i] = new Rect(0, 0, 0, 0);
            }

            // 准备界面的 CameraImagViewForPersonX 固定挂在 PlayerX-1 节点下。
            // 人数不足 4 时业务只显示前 N 个 Player 节点，因此这里必须按座位索引写入对应显示槽，
            // 避免 2 人局把画面写入 Player2、Player3 下方后被隐藏父节点挡掉。
            for (int seatIndex = 0; seatIndex < maxSlots; seatIndex++)
            {
                var parseDataDemo = AndroidParseDataDemo.Instance;
                if (parseDataDemo != null && !parseDataDemo.IsSlotEnabled(seatIndex))
                {
                    continue;
                }

                if (seatIndex < rowCount)
                {
                    float left = personPlayerRectf[seatIndex, 0];
                    float top = personPlayerRectf[seatIndex, 1];
                    float right = personPlayerRectf[seatIndex, 2];
                    float bottom = personPlayerRectf[seatIndex, 3];
                    if (IsValidPersonRect(left, top, right, bottom))
                    {
                        try
                        {
                            Rect targetRect = new Rect(left, top, right - left, bottom - top);
                            _cameraTextureViewRectPartitionView[seatIndex] = targetRect;
                        }
                        catch (Exception e)
                        {
                            Debug.LogError($"更新人物 {seatIndex} 纹理时出错: {e.Message}");
                        }
                    }
                }
            }
    ...
    public void DrawSkeleton(int personIndex, float[,] kptPoseList, float[,] kptLHandList, float[,] kptRHandList, float left, float top, float right, float bottom, int playerId, float score)
    {
        if (Playerskeletons.Length > personIndex)
        {
            ...
            if (bridge != null)
            {
                ...
                if (isPartitionView)
                {
                    // PartitionView 模式下，准备界面的小图挂在对应 Player 节点下。
                    // 座位0对应 cameraViews[1]，座位1对应 cameraViews[2]，依次类推。
                    ...
                    int cameraViewCount = bridge.GetCameraViewCount();
                    viewIndex = Mathf.Clamp(slotIndex + 1, 0, cameraViewCount - 1);
                }
                else
                {
                    viewIndex = 0;
