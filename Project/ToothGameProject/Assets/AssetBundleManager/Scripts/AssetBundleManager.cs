...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : IDisposable
    {
        ...
        private void GetManifest(string bundleName, bool getFreshManifest, Action<AssetBundle> onComplete)
        {
            ...
            GetManifestpublic(bundleName, manifestVersion, 0);
        }

        private void GetManifestpublic(string manifestName, uint version, int uriIndex)
        {
            handler = new AssetBundleDownloader(baseUri[0], baseUri[1], baseUri[2], m_CodeConfig);
            ...
            handler.Handle(new AssetBundleDownloadCommand {
                ...
                OnComplete = manifest => {
                    ...
                    if (manifest == null && uriIndex < maxIndex && version > 1) {
                        ...
                        GetManifestpublic(manifestName, version, uriIndex + 1);
                        ...
                        GetManifestpublic(manifestName, version - 1, uriIndex);
                    } else {
        ...
        public void GetBundle(string bundleName, bool fullPath, Action<AssetBundle> onComplete)
        {
            ...
            GetBundle(bundleName, fullPath, onComplete, DownloadSettings.UseCacheIfAvailable);
        }
        ...
        /// </param>
        public void GetBundle(string bundleName, bool fullPath, Action<AssetBundle> onComplete, DownloadSettings downloadSettings)
        {
            if (Initialized == false) {
                Debug.LogError("AssetBundleManager must be initialized before you can get a bundle.");
                onComplete(null);
            ...
            if (dependenciesToDownload.Count > 0) {
                ...
                for (int i = 0; i < dependenciesToDownload.Count; i++)
                {
                    ...
                    GetBundle(dependencyName, fullPath, onDependenciesComplete);
                }
            } else {
                handler.Handle(mainBundle);
            }
...
//        /// </summary>
//        /// <param name="bundleName">Name of the bundle to download.</param>
//        public async Task<AssetBundle> GetBundle(string bundleName, bool fullPath)
//        {
...
//            GetBundle(bundleName, fullPath, onComplete);
//            return await completionSource.Task;
//        }
...
//        ///     regardless of this setting.  If it's important that a new version is downloaded then be sure it isn't active.
//        /// </param>
//        public async Task<AssetBundle> GetBundle(string bundleName, bool fullPath, DownloadSettings downloadSettings)
//        {
//            var completionSource = new TaskCompletionSource<AssetBundle>();
//            var onComplete = new Action<AssetBundle>(bundle => completionSource.SetResult(bundle));
//            GetBundle(bundleName, fullPath, onComplete, downloadSettings);
//            return await completionSource.Task;
//        }
...
//        {
//            try {
//                await GetBundle(bundleName, false);
...
//#endif
        ...
        /// <returns></returns>
        public AssetBundleAsync GetBundleAsync(string bundleName, bool fullPath)
        {
            if (Initialized == false) {
                Debug.LogError("AssetBundleManager must be initialized before you can get a bundle.");
            ...
            return new AssetBundleAsync(bundleName, fullPath, GetBundle);
        }
        ...
        public void UnloadBundle(AssetBundle bundle)
        {
            ...
            UnloadBundlepublic(useHash ? GetHashedBundleName(bundle.name) : bundle.name, false, false);
        }
        ...
        {
            if (bundle == null) return;
            UnloadBundlepublic(useHash ? GetHashedBundleName(bundle.name) : bundle.name, unloadAllLoadedObjects, false);
        }
        ...
        public void UnloadBundle(string bundleName, bool unloadAllLoadedObjects, bool force)
        {
            ...
            UnloadBundlepublic(useHash ? GetHashedBundleName(bundleName) : bundleName, unloadAllLoadedObjects, force);
        }

        /// <summary>Unloads an AssetBundle and its dependencies if there are no more active references.</summary>
        private void UnloadBundlepublic(string bundleName, bool unloadAllLoadedObjects, bool force)
        {
            ...
            if (force || --cache.References <= 0) {
                ...
                for (int i = 0; i < cache.Dependencies.Length; i++) {
                    UnloadBundlepublic(cache.Dependencies[i], unloadAllLoadedObjects, force);
                }
            }
        }
        ...
        private string m_CodeConfig = "";
        public void SetCodeConfig(string code_config)
        {
            m_CodeConfig = code_config;
        }
