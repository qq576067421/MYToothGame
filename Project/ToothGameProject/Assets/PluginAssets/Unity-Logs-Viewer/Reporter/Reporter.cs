...
public class Reporter : MonoBehaviour
{
	...
	public float fps;
    public string OpTips = "顺时针或者逆时针拖动旋转10圈弹出调试信息";
	...
	public List<int> Passwords = new List<int>() { 1,2,3,4,3,4,1,4 };
	public void SetPasswords(List<int> passwords)
	{
		this.Passwords.Clear();
		this.Passwords.AddRange(passwords);
	}
	...
	void addSample()
	{
		...
		sample.fps = fps;
		sample.fpsText = fps.ToString();
#if UNITY_CHANGE3
		sample.loadedScene = SceneManager.GetActiveScene().buildIndex;
#else
	...
	public void Initialize()
	{
        ...
        DebugConfigContent = new GUIContent("", images.infoImage, "DebugConfig about application");
		...
		m_HeightScale = PlayerPrefs.GetFloat("debug_config_screen_height", 1.0f);
		if (show || DefaultShow)
        {
			doShow();
		}
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		GUILayout.Label(fps.ToString(), nonStyle, GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
    ...
    void drawInfo_enableDisableToolBarButtons()
	{
		...
		tempRect = GUILayoutUtility.GetLastRect();
		GUI.Label(tempRect, fps.ToString(), lowerLeftFontStyle);
	...
	void drawToolBar()
	{
        ...
        if (GUILayout.Button(DebugConfigContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            currentView = ReportView.DebugConfig;
        }
	...
	public void OnGUIDraw()
	{
		...
		screenRect.y = 0;
		screenRect.width = Screen.width;
		screenRect.height = Screen.height * m_HeightScale;
		...
		logsRect.height = Screen.height * m_HeightScale * 0.75f - size.y * 2f;

		stackRectTopLeft.x = 0f;
		stackRect.x = 0f;
		stackRectTopLeft.y = Screen.height * m_HeightScale * 0.75f;
		stackRect.y = Screen.height * m_HeightScale * 0.75f;
		stackRect.width = Screen.width;
		stackRect.height = Screen.height * m_HeightScale * 0.25f - size.y;



		detailRect.x = 0f;
		detailRect.y = Screen.height * m_HeightScale - size.y * 3;
    ...
    private int GetArea(Vector2 pos)
    {
    ...
    private void AddArea(Vector2 p)
    {
        int area = GetArea(p);
        if (gestureDetector.Count == 0)
        {
            gestureDetector.Add(area);
        }
        else
        {
            int lastArea = gestureDetector[gestureDetector.Count - 1];
            if(lastArea != area)
            {
                gestureDetector.Add(area);
            }
    ...
    List<int> gestureDetector = new List<int>();
	int gestureCount = 0;
	bool isGestureDone()
	{
        ...
        if(gestureDetector.Count == Passwords.Count)
        {
            int count = gestureDetector.Count;
            for(int i=0;i <count; ++i)
            {
                if( gestureDetector[i] != Passwords[i])
                {
                    return false;
                }
            }
	...
	void Update()
	{
		...
		if( sceneIndex != -1 && scenes != null && string.IsNullOrEmpty( scenes[sceneIndex] ))
