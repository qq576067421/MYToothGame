    ...
    public class RFLimitations
    {
        ...
        public bool CollisionCheck(RayfireRigid rigid)
        {
            if (rigid.limitations.byCollision == true)
                return true;
            if (rigid.damage.enable == true && rigid.damage.collect == true)
                return true;
            return false;
        }
        ...
        public bool DamagePointsCheck(Collision collision, RayfireRigid rigid)
        {
            ...
            for (int i = 0; i < collision.contactCount; i++)
            {
                ...
                if (rigid.ApplyDamage (collisionMagnitude * rigid.damage.multiplier, contactVector3, 0f, collision.contacts[i].thisCollider) == true)
                        return true;
            }
