...
namespace RayFire
{
    ...
    public class RFParticles
    {
            ...
            for (int j = rigid.clusterDemolition.cluster.shards.Count - 1; j >= 0; j--)
            {
                ...
                if (rigid.clusterDemolition.cluster.shards[j].neibShards.Count < rigid.clusterDemolition.cluster.shards[j].nAm)
                {
                    ...
                    else if (rigid.meshRoot != null)
                    {
                        ...
                        //Debug.Log (rigid.clusterDemolition.cluster.shards[j].Visible);
        ...
        {
            // Create ps
            debris.pSystem      = CreateParticleSystem (tm, debris.rendering);
        ...
        {
            // Create ps
            dust.pSystem      = CreateParticleSystem (tm, dust.rendering);
        ...
        // Create host and particle system
        static ParticleSystem CreateParticleSystem(Transform tm, RFParticleRendering rend)
        {
            ...
            if (rend.t == true)
            {
                if (string.IsNullOrEmpty (rend.tag))
                    rend.tag = "Untagged";
                ps.gameObject.tag = rend.tag;
            }
            if (rend.l == true)
                ps.gameObject.layer = rend.layer;
