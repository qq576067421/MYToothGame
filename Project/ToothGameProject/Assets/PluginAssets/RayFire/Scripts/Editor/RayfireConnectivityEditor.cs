...
namespace RayFire
{
    ...
    public class RayfireConnectivityEditor : Editor
    {
        ...
        static GUIContent gui_filt_seed = new GUIContent ("  Seed",                 "Seed for random percentage.");
        ...
        void UI_Collapse()
        {
            ...
            if (exp_collapse == true)
            {
                ...
                if (conn.collapse.type != RFCollapse.RFCollapseType.Random)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    conn.collapse.var = EditorGUILayout.IntSlider (gui_clp_var, conn.collapse.var, 0, 100);
                    if (EditorGUI.EndChangeCheck() == true)
                        foreach (RayfireConnectivity scr in targets)
                        {
                            scr.collapse.var = conn.collapse.var;
                            SetDirty (scr);
                        }
                }
                ...
                EditorGUI.BeginChangeCheck();
                conn.collapse.seed = EditorGUILayout.IntSlider (gui_clp_seed, conn.collapse.seed, 0, 99);
                if (EditorGUI.EndChangeCheck() == true)
                    foreach (RayfireConnectivity scr in targets)
                    {
                        scr.collapse.seed = conn.collapse.seed;
                        SetDirty (scr);
                    }
