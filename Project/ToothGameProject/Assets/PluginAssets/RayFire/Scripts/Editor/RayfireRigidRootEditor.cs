    ...
    public class RayfireRigidRootEditor : Editor
    {
        ...
        void UI_Physic()
        {
            ...
            if (exp_phy == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.mt = (MaterialType)EditorGUILayout.EnumPopup (gui_phy_mtp, root.physics.mt);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.mt = root.physics.mt;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.ct = (RFColliderType)EditorGUILayout.EnumPopup (gui_phy_ctp, root.physics.ct);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.ct = root.physics.ct;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.pc = EditorGUILayout.Toggle (gui_phy_pln, root.physics.pc);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.pc = root.physics.pc;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.ine = EditorGUILayout.Toggle (gui_phy_ign, root.physics.ine);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.ine = root.physics.ine;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.gr = EditorGUILayout.Toggle (gui_phy_grv, root.physics.gr);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.gr = root.physics.gr;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.si = EditorGUILayout.IntSlider (gui_phy_slv, root.physics.si, 1, 20);
                if (EditorGUI.EndChangeCheck() == true)
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.si = root.physics.si;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.st = EditorGUILayout.Slider (gui_phy_slt, root.physics.st, 0.001f, 0.1f);
                if (EditorGUI.EndChangeCheck() == true)
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.st = root.physics.st;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.physics.dm = EditorGUILayout.Slider (gui_phy_dmp, root.physics.dm, 0f, 1f);
                if (EditorGUI.EndChangeCheck() == true)
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.physics.dm = root.physics.dm;
                        SetDirty (scr);
                    }
