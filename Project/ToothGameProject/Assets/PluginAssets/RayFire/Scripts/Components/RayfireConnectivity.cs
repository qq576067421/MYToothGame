...
namespace RayFire
{
    ...
    public class RayfireConnectivity : MonoBehaviour
    {
        ...
        void Reset()
        {
            ...
            backup            = null;
            initShardAmount   = 0;
            connectivityEvent = new RFConnectivityEvent();
            joints            = new RFJointProperties();
            expand            = 0.5f;
        }
        */
        ...
        void Clusterize()
        {
            for (int i = 0; i < cluster.childClusters.Count; i++)
            {
                ...
                //Debug.Log (cluster.childClusters[i].tm.name);
                //Debug.Log (cluster.childClusters[i].shards.Count);
