    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
				...
				else if ( abs(_RoundMode - 1) < 0.1)
				{
					//计算X轴方向距离
					float disX = distance(IN.worldPosition.x, _Center.x) * 2;
					//计算Y轴方向距离
					float disY = distance(IN.worldPosition.y, _Center.y) * 2;
