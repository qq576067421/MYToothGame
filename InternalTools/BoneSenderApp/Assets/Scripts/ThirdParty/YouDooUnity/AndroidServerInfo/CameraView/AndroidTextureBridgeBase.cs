...
public abstract class AndroidTextureBridgeBase : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        Debug.Log($"[Diagnostic] Is Vulkan Supported: {IsSupportedGraphicsBackend()}");

        if (!IsSupportedGraphicsBackend())
        {
    ...
    private void ApplyRegionToViews(Rect[] rects)
    {
        ...
        if (isPartitionView)
        {
            ...
            // PartitionView 模式：使用 cameraViews 的 1~4 (即索引 1 到 length-1)
            for (int i = 1; i < cameraViews.Length; i++)
            {
                CameraTextureView view = cameraViews[i];
                if (view == null) continue;
                // 因为 cameraViews 是从 1 开始，对应玩家的索引是 i-1
                int playerIndex = i - 1;
